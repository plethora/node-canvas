...
void Canvas::Initialize(Nan::ADDON_REGISTER_FUNCTION_ARGS_TYPE target)
{
  ...
  Nan::SetPrototypeMethod(ctor, "getWrittenBounds", GetWrittenBounds);
  Nan::SetPrototypeMethod(ctor, "copyRect", CopyRect);
...
void Canvas::ToPngBufferAsync(uv_work_t *req)
{
...
#ifdef HAVE_JPEG
void Canvas::ToJpegBufferAsync(uv_work_t *req)
{
...
void Canvas::ToBufferAsyncAfter(uv_work_t *req)
{
  Nan::HandleScope scope;
...
#if CAIRO_VERSION >= CAIRO_VERSION_ENCODE(1, 16, 0)
      if (info[1]->IsObject())
      { // toBuffer("application/pdf", config)
...
streamPNG(void *c, const uint8_t *data, unsigned len)
{
  ...
  Local<Value> argv[3] = {
      Nan::Null(), buf, Nan::New<Number>(len)};
  closure->cb.Call(sizeof argv / sizeof *argv, argv, &async);
...
NAN_METHOD(Canvas::StreamPNGSync)
{
  ...
  {
    Local<Value> argv[3] = {
        Nan::Null(), Nan::Null(), Nan::New<Uint32>(0)};
    Nan::Call(closure.cb, Nan::GetCurrentContext()->Global(), sizeof argv / sizeof *argv, argv);
  }
  return;
}
...
NAN_METHOD(Canvas::RegisterFont)
{
  ...
  if (!sys_desc)
    return Nan::ThrowError("Could not parse font file");
...
NAN_METHOD(Canvas::GetWrittenBounds)
{
  if (!info[0]->IsNumber())
  {
    return Nan::ThrowError("Wrong argument type");
  ...
  uint32_t background = Nan::To<uint32_t>(info[0]).FromMaybe(0);

  Canvas *canvas = Nan::ObjectWrap::Unwrap<Canvas>(info.This());
  cairo_surface_t *surface = canvas->surface();
  const uint32_t *data = (const uint32_t *)cairo_image_surface_get_data(surface);
  auto width = uint32_t(canvas->getWidth());
  auto height = uint32_t(canvas->getHeight());
  auto min_x = width;
  auto min_y = height;
  auto max_x = 0u;
  auto max_y = 0u;
  ...
  if (found)
  {
    auto jsArr = Nan::New<v8::Array>(4);
    jsArr->Set(0, Nan::New(min_x));
    jsArr->Set(1, Nan::New(min_y));
    jsArr->Set(2, Nan::New(max_x));
    jsArr->Set(3, Nan::New(max_y));
    info.GetReturnValue().Set(jsArr);
  }
}

NAN_METHOD(Canvas::ClearRect)
{
  if (!info[0]->IsArray())
  {
    return Nan::ThrowError("Wrong argument type");
  ...
  if (jsArr->Length() < 4)
  {
    return Nan::ThrowError("Wrong argument length");
  }
  uint32_t background = Nan::To<uint32_t>(info[1]).FromMaybe(0);
  ...
  auto width = uint32_t(canvas->getWidth());
  auto x0 = (uint32_t)jsArr->Get(0)->NumberValue();
  auto y0 = (uint32_t)jsArr->Get(1)->NumberValue();
  auto x1 = (uint32_t)jsArr->Get(2)->NumberValue();
  auto y1 = (uint32_t)jsArr->Get(3)->NumberValue();

  for (auto j = y0; j < y1; ++j)
  {
    auto row_data = data + j * width + x0;
...
NAN_METHOD(Canvas::CopyRect)
{
  ...
  for (auto j = y0; j < y1; ++j)
  {
    ...
    {
      auto pix = *row_data;
      auto a = 0xff000000 & pix;
      auto r = 0x00ff0000 & pix;
      auto g = 0x0000ff00 & pix;
      auto b = 0x000000ff & pix;
      auto res = a | g | (r >> 16) | (b << 16);
      pixels.push_back(res);
      ++row_data;
    }
  }

  Isolate *iso = Nan::GetCurrentContext()->GetIsolate();
  Local<Object> buf = node::Buffer::Copy(iso, reinterpret_cast<const char *>(pixels.data()), pixels.size() * 4).ToLocalChecked();
  info.GetReturnValue().Set(buf);
}
...
void Canvas::resurface(Local<Object> canvas)
{
  Nan::HandleScope scope;
  Local<Value> context;
